'''Calculate trajectories for ice stations (mobile with the ice) or
...
Created by Emily Down (emilyjd _AT_ met.no) at Met Norway based on
code by Thomas Lavergne, funded by the SUDARCO project.
...
def parse_args():
    ...
    p.add_argument('-e', '--enddate', required=False, default=None,
                   help="Date as YYYYmmdd. This is the fixed date at "
    ...
    p.add_argument('-fe', '--firstenddate', required=False, default=None,
                   ...
                   "trajectories are required. In YYYYmmdd format. "
                   "Can be a comma-separated list corresponding to each "
                   "ID, matching the length of that list")
    ...
    p.add_argument('-k', '--maxskip', required=False, default=4,
                   help="Maximum number of days to skip if ice "
...
def retrieve_icedrift_file(dt, hemi='nh', icedrift_v=valid_vers['drift'][0],
                           maxskip=4, timestep=1, forwardtrack=False,
                           verbose=False):
...
def read_icedrift_file(driftfile, pdate, dtimestep=1, timestep=1):
    ...
    with Dataset(driftfile, 'r') as dataset:
        ...
        if check_vars_in_dataset(reqvarmet, dataset):
            ...
            # Since we want to go back in time by default, return the
            # opposite of the icedrift vector.
        ...
        # Read in NSDIC ice drift file
            ...
            # position for these rather than the start position
...
def find_iceconc_file(dt, hemi='nh', iceconc_v=valid_vers['conc'][0],
                      maxskip=4, forwardtrack=False, verbose=False):
    '''Locate an ice concentration file for the correct version and date'''

    # Look for the file up to maxskip days back in time (or forward in time
...
def iceconc_areadef_lmask(dt, hemi='nh', iceconc_v=valid_vers['conc'][0],
                          maxskip=4, forwardtrack=False, verbose=False):
...
def get_ice_conc(dt, area_def, lmask, iceconc_v=valid_vers['conc'][0],
                 maxskip=4, forwardtrack=False, verbose=False):
...
def extract_iceconc(dt, lons, lats, area_def, lmask,
                    iceconc_v=valid_vers['conc'][0], maxskip=4,
...
# ################################################################
# MOTION TRACKING AND CONCENTRATION CONDITION CHECKING
# ################################################################
...
def track_traj(enddates, endlons, endlats, begindates, periods,
               forwardtrack=False, hemi='nh',
    ...
    # overwritten by the drift timestep in a moment)
    ...
    area_def = utils.load_grid_defs_from_OSISAF_ncCF_file(drift_file,
                                                      verbose=verbose)
    area_def.area_id = hemi

    # Setting up the timestep required for the tracking. At the moment
    # this is always 1 day
    timestep = 1
    # Initialize tracking
    arrlen = len(endlons)
    # The status array keeps track of trajectory status. This is only a 1-d
    # array as long as the number of ice stations or moorings, so does not
    # keep status info for all timesteps, but only the most recent. Status 0
    # is that this trajectory has not begun yet. Status 1 is a trajectory in
    # progress. Status 2 is a completed trajectory.
    ...
    while valid_date and not np.all(status == 2):
        ...
        utils.print_percentage(loopcount, maxper)

        # Check if any new trajectories begin at this timepoint, and if so
        # the concentration needs to be initialised
        ...
        lons[trp, trajstep[trp] + 1], lats[trp, trajstep[trp] + 1], \
            drift_skip = track_single_step(tpoint,
        ...
        if ex_lmask is not None:
            ...
            land_points[trp] = pr.kd_tree.resample_nearest(ex_lmask_swath,
                                                    ...
                                                    radius_of_influence=150000,
                                                      fill_value=None)
            # The land points are incremented if over land, zeroed when
            # back over water, and when a trajectory reaches > ow_repeat
            # land counts, it is closed
        ...
        if ow_threshold_mid is not None:
            ...
            # Flag all later timesteps after the mid open water threshold has
            # been exceeded for more than ow_repeat steps (subtracting off
            # these repeated steps)
...
def track_loc(bid=None, enddate=None, firstenddate=None, elon=None,
    ...
    if json_file:
        ...
        # Dateify the dates
        enddate = [datetime.strptime(str(x), "%Y%m%d") for x in enddate]
        enddate = np.array(enddate)
        if mode == '2d':
            firstenddate = [datetime.strptime(str(x), "%Y%m%d")
    ...
    if mode == '1d':
        ...
        print("{}: Running tracking in 1-D mode".format(bid[0]))
        ...
        print("{}: Running tracking in 2-D mode".format(bid[0]))
    ...
    enddate_1d = np.array([utils.convtodt(x) for x in enddate_1d])
    begindate_1d = np.array([utils.convtodt(x) for x in begindate_1d])

    # Find the output name if it is not specified already
    ...
    btoutput = track_traj(enddate_1d, elon_1d, elat_1d, begindate_1d,
                          per_1d,
                          forwardtrack=forwardtrack,
                          hemi=hemi,
                          iceconc_v=iceconc_v,
    ...
    flagarr = np.full(dimdata, -1, dtype=int)
    ...
    concdirs = ','.join(set(utils.strip_date(allconcdir)))
    driftdirs = ','.join(set(utils.strip_date(alldriftdir)))
    ...
    with Dataset(outname, 'w', format='NETCDF4') as outf:
        ...
        dimreferencedate = outf.createDimension('referencedate', maxtimespan)
        lldims = ('id', 'referencedate', 'timestep')
        ...
        timestepvar = outf.createVariable(varname='timestep',
                                          ...
                                          datatype='int32')
        if forwardtrack:
            timestepvar.long_name = 'Timestep of days forwards in time'
        else:
        ...
        enddatevar = outf.createVariable(varname='referencedate',
                                         dimensions=('id', 'referencedate'),
                                         datatype='float64')
        enddatevar.standard_name = 'time'
        enddatevar.long_name = 'Reference datetime of the trajectory'
        ...
        # Global metadata
        if forwardtrack:
            outf.title = "Forward tracked trajectories of on-ice positions"
            outf.summary = "Forward tracked trajectories of on-ice positions. With each daily timestep forwards in time, the position is shifted according to the sea-ice drift from that date and location, and compared to the sea-ice concentration from that date and location. Trajectories are ended when the calculated position is in a region of sea-ice concentration less than the threshold limit for a certain consecutive number of days, when the set timelimit for tracking is reached, or when the current date is reached."
        else:
            outf.title = "Backtracked trajectories of on-ice positions"
            outf.summary = "Backtracked trajectories of on-ice positions. With each daily timestep backwards in time, the position is shifted according to the sea-ice drift from that date and location, and compared to the sea-ice concentration from that date and location. Trajectories are ended when the calculated position is in a region of sea-ice concentration less than the threshold limit for a certain consecutive number of days, or when the set timelimit for tracking is reached."            
        outf.comment = "The code to prepare this file was funded by the SUDARCO project. Please credit use with '<Data/plot> prepared with use of code funded by the SUDARCO project and prepared at Met Norway'."
        ...
        outf.time_coverage_start = "{:%Y%m%d}".format(mindate)
        outf.time_coverage_end = "{:%Y%m%d}".format(maxdate)
        ...
        if period.endswith('d'):
            ...
            perunit = 'years'
        outf.max_tracking_period = '{} {} (maximum time period over which positions are tracked)'.format(perlen, perunit)
